#!/bin/bash
set -e
echo "🔥 Исправляем черные символы, свечение и перезаписываем SVG..."

# Перезаписываем SVG (чистый код, без багов)
...
/**
 * 🐦‍🔥 Глубинное парение v5.4 — Абсолютная стабильность (Золотое свечение без багов)
 */
(function() {
    const sacredData = [
        { icon: '/shapes/bg3.svg' },
    ...
    function spawnSymbol() {
        ...
        const size = 120 + Math.random() * 40; // Идеальный размер (120-160px)
        // Адаптивное смещение от низа: умная формула для всех экранов
        const bottomOffset = Math.max(window.innerHeight * 0.12, 80) + Math.random() * 40; 

        const el = document.createElement('div');
        el.className = 'sacred-floating-symbol';

        // Золотое свечение постоянно. Размытие будет идти через переходы transform/opacity
        el.style.cssText = `
            position: absolute;
            left: 50%;
            ...
            opacity: 0;
            pointer-events: none;
            transition: transform 4s cubic-bezier(0.2, 0.9, 0.3, 1.1), opacity 3.5s ease-in-out;
        `;

        const img = document.createElement('img');
        img.src = symbol.icon;
        img.alt = '';
        // Ключевой фикс для браузеров: magic filter превращает черный цвет в золотой свет
        img.style.cssText = `
            width: 100%; height: 100%; pointer-events: none; background: transparent;
            filter: drop-shadow(0 0 30px rgba(255, 215, 0, 0.7)) drop-shadow(0 0 60px rgba(167, 139, 250, 0.4));
        `;
        el.appendChild(img);
