// AI-1 Control Dashboard — визуализация эволюции
...
app.use(express.static(path.join(ROOT, 'public_dashboard')));

// === API: Leaderboard ===
app.get('/api/leaderboard', (req, res) => {
  try {
    const files = fs.readdirSync(path.join(ROOT, 'memory/patterns'))
    ...
    for (const f of files) {
      const d = JSON.parse(fs.readFileSync(path.join(ROOT, 'memory/patterns', f), 'utf8'));
...
app.get('/api/races', (req, res) => {
  try {
    const dir = path.join(ROOT, 'memory/patterns');
...
app.get('/api/skills', (req, res) => {
  try {
    const skills = fs.readdirSync(path.join(ROOT, 'skills'))
      ...
      .map(f => {
        const content = fs.readFileSync(path.join(ROOT, 'skills', f), 'utf8');
...
app.get('/api/stats', (req, res) => {
  try {
    const racesCount = fs.readdirSync(path.join(ROOT, 'memory/patterns'))
      .filter(f => f.startsWith('race_')).length;
    const skillsCount = fs.readdirSync(path.join(ROOT, 'skills')).filter(f => f.endsWith('.md')).length;
    const patternsCount = fs.readdirSync(path.join(ROOT, 'memory/patterns')).filter(f => f.endsWith('.json')).length;
    const everosSize = '—';
...
app.get('/api/cache', (req, res) => {
  try {
    const logPath = path.join(ROOT, 'logs/usage.log');
...
app.get('/radio', (req, res) => res.sendFile(path.join(ROOT, 'public_dashboard', 'radio.html')));
app.get('/', (req, res) => res.sendFile(path.join(ROOT, 'public_dashboard', 'index.html')));
...
app.get('/api/logs', (req, res) => {
  ...
  const logPath = path.join(ROOT, 'logs', file + (file.endsWith('.log') ? '' : '.log'));
  try {
...
app.get('/api/logs-list', (req, res) => {
  try {
    const files = fs.readdirSync(path.join(ROOT, 'logs'))
      ...
      .map(f => {
        const stat = fs.statSync(path.join(ROOT, 'logs', f));
...
server.listen(PORT, () => console.log('AI-1 Control Dashboard (with socket.io): http://localhost:' + PORT));
