#!/bin/bash
set -e
echo "🔥 Устанавливаем чистые SVG и строгую привязку к зоне под 'Соратником'..."

# 1. Перезаписываем SVG идеально чистыми эталонными кодами (БЕЗ лишних пунктиров и точек)
...
# 2. Перезаписываем JS с жесткой привязкой к карточке "Соратник"
cat > /home/ishidin/phoenix/public/js/sacred-carousel.js << 'ENDOFFILE2'
/**
 * 🐦‍🔥 Глубинное парение — v6.1 Строгая зона (только под Соратником)
 */
(function() {
    const sacredData = [
        { icon: '/shapes/bg3.svg' },
...
pm2 restart phoenix
echo "✅ ВСЕ ИСПРАВЛЕНО. Лишние пунктиры убраны. Символы появляются строго под 'Соратником'."
