#!/usr/bin/env node
// evolution/puppeteer.js
// ─────────────────────────────────────────────────────────────────────────────
// КУКЛОВОД (Puppeteer) — RL-агент, управляющий пулом пилотов.
//
// По мотивам «Кукловод» (NeurIPS 2025): один управляющий агент держит нити
// пула — решает, запускать ли гонку, как расставить приоритеты и какую
// стратегию включить, чтобы максимизировать суммарную награду пула.
//
// Пространство наблюдений (observation):
//     {
//       pool_size : number,        // сколько пилотов в пуле
//       load      : number,        // загрузка пула, 0..1 (или 0..100)
//       budget    : number|object, // остаток бюджета либо { total, remaining }
...
//     change_priority    — переставить приоритеты задач в пуле (дёшево);
...
//
// Политика: ЗАГЛУШКА rule-based (детерминированные правила ниже). Поверх неё
// есть каркас табличного Q-learning (epsilon-greedy) — он включается только
// когда таблица «прогрета» нужным числом визитов. По умолчанию source='rule'.
//
// Публичный API:
//     const { Puppeteer, createPuppeteer } = require('./evolution/puppeteer');
...
//
// Зависимости: только Node core (fs, path). Никаких побочных эффектов при
// require() — файл безопасно грузится для node --check и юнит-тестов.
// ─────────────────────────────────────────────────────────────────────────────
...
const DEFAULTS = Object.freeze({
  ...
  lowBudget: 0.2, // доля остатка бюджета: ниже — режим экономии
  richBudget: 0.5, // доля остатка бюджета: выше — бюджет богатый
  epsilon: 0.1, // доля случайных действий (Q-политика)
  alpha: 0.3, // скорость обучения Q
  gamma: 0.9, // дисконт Q
  minVisits: 8, // сколько раз посетить (s,a) до доверия к Q
  qWeight: 0.6, // вес Q-совета в hybrid-режиме
  weights: Object.freeze({ quality: 0.5, budget_efficiency: 0.3, stability: 0.2 }),
  qTableFile: path.join(ROOT, 'memory', 'puppeteer_qtable.json'),
});
...
/** Нормированный наклон линейной регрессии по точкам (в единицах y на шаг). */
...
/**
 * Нормализовать сырые метрики входа в каноничную обсервацию.
 * Устойчиво к мусору: любые пропуски заменяются безопасными значениями.
 */
function normalizeObservation(raw = {}) {
  ...
  if (rawBudget && typeof rawBudget === 'object') {
    ...
    budgetLeft = toNum(rawBudget.remaining ?? rawBudget.left ?? rawBudget.budget_left, budgetLeft);
  ...
  // Стабильность = 1 - коэффициент вариации по качеству (кламп 0..1).
  let stability = 1;
 ...
 * Если метрика задана явно — используется она; иначе выводим из spent/budget.
 */
...
/**
 * Награда = w_q*quality + w_e*budget_efficiency + w_s*stability + penalty.
...
function computeReward(metrics = {}, weights = DEFAULTS.weights) {
  ...
  if (metrics.stability != null) {
    ...
    const previous = Array.isArray(metrics.history)
      ? metrics.history.map((h) => clamp01(h && typeof h === 'object' ? h.quality : h))
      : [];
...
class Puppeteer {
  ...
  constructor(options = {}) {
    ...
    /** Q-таблица: key = `${stateKey}::${action}` -> { value, visits } */
    this.q = new Map();
    this.steps = 0;
    this.races = 0;
    this.totalReward = 0;
  ...
  /** @param {object} raw сырое состояние пула @returns {object} каноничная обсервация */
  observeState(raw = {}) {
  ...
  /**
   * Детерминированная rule-based политика.
  ...
  rulePolicy(obs) {
    ...
    if (s.load >= c.maxLoad) {
      if (this.strategy !== 'throttle') {
        return this._makeAction(ACTIONS.SWITCH_STRATEGY, { strategy: 'throttle' },
          `load=${round(s.load, 3)} >= maxLoad=${c.maxLoad}: включаю throttle`, 'rule');
      }
      return this._makeAction(ACTIONS.CHANGE_PRIORITY, { priority: 'deadline_first' },
        'throttle уже активен: переприоритизирую на deadline_first', 'rule');
    }

    // 2. Бюджет на исходе → экономим.
    if (s.budget_ratio <= c.lowBudget) {
      if (this.strategy !== 'budget_first') {
        return this._makeAction(ACTIONS.SWITCH_STRATEGY, { strategy: 'budget_first' },
          `budget_ratio=${round(s.budget_ratio, 3)} <= ${c.lowBudget}: режим экономии`, 'rule');
      }
      return this._makeAction(ACTIONS.CHANGE_PRIORITY, { priority: 'cheap_first' },
        'бюджет исчерпан: сначала дешёвые задачи', 'rule');
    }

    // 3. Качество падает → исследуем пространство стратегий.
    if (s.trend === 'down' && this.strategy !== 'explore') {
      return this._makeAction(ACTIONS.SWITCH_STRATEGY, { strategy: 'explore' },
        `trend=down (slope=${s.slope}): пробую explore`, 'rule');
    }

    // 4. Есть ресурсы и бюджет → запускаем гонку.
    if (
      s.load < c.busyLoad &&
      s.pool_size >= c.minPool &&
      s.budget_ratio > c.lowBudget
    ) {
      const boxes = Math.max(1, Math.min(s.pool_size, Math.round(s.pool_size * (1 - s.load)) || 1));
      const priority = s.quality_mean < 0.5 ? 'quality_first' : 'quality_first';
      return this._makeAction(ACTIONS.START_RACE, { boxes, priority, strategy: this.strategy },
    ...
    return this._makeAction(ACTIONS.CHANGE_PRIORITY, { priority },
      'устойчивое состояние: уточняю приоритеты задач', 'rule');
  }
  ...
  qPolicy(obs) {
    ...
    return this._makeAction(best, this._defaultParams(best, s), `Q[${key}]=${round(bestVal, 3)}`, 'q');
  }
  ...
  _defaultParams(action, s) {
    ...
    if (action === ACTIONS.CHANGE_PRIORITY) {
      return { priority: s.budget_ratio > this.config.richBudget ? 'quality_first' : 'cheap_first' };
    }
  ...
  decide(rawState, opts = {}) {
    ...
    if (mode === 'q') {
      ...
      if (!chosen) {
        // холодный старт: исследуем случайно
      ...
      if (q && rng() > this.config.epsilon) {
        chosen =
          q.action === rule.action
            ? q
            : rng() < this.config.qWeight
              ? q
              : rule;
      } else {
        chosen = rule;
      ...
      // rule: epsilon-шум не добавляем — заглушка детерминирована
   ...
   * @param {object} [nextState] следующее наблюдение (для bootstrap)
  ...
  /** Ручное обновление Q (для тестов/офлайн-обучения). */
  learn(stateKey, action, reward, nextStateKey = null) {
    const nk = nextStateKey;
    let bestNext = 0;
    if (nk) bestNext = Math.max(...ACTION_LIST.map((a) => this._qValue(nk, a)));
    const key = `${stateKey}::${action}`;
    const rec = this.q.get(key) || { value: 0, visits: 0 };
  ...
  /**
   * Прогнать один шаг «решить → применить → получить награду».
   * @param {object} rawState
   * @param {Function} apply функция применения действия -> метрики
   * @returns {Promise<{action:object, reward:number, metrics:object, nextState:object}>}
   */
  ...
  /** Прогнать N шагов над переданным «средой»-резолвером. */
  ...
  _loadQTable() {
    try {
      if (!this.config.qTableFile || !fs.existsSync(this.config.qTableFile)) return;
      const data = JSON.parse(fs.readFileSync(this.config.qTableFile, 'utf8'));
      if (Array.isArray(data.q)) {
        for (const rec of data.q) {
          if (rec && rec.key) this.q.set(rec.key, { value: toNum(rec.value, 0), visits: toNum(rec.visits, 0) });
        }
      }
      if (STRATEGIES.includes(data.strategy)) this.strategy = data.strategy;
    } catch (e) {
      /* битая таблица не должна ломать конструктор */
    }
  }

  saveQTable(file = this.config.qTableFile) {
    try {
...
// ─────────────────────────────────────────────────────────────────────────────
// CLI: node evolution/puppeteer.js --demo [--steps 20] [--json]
// ─────────────────────────────────────────────────────────────────────────────
...
function makeSyntheticEnv(seedState) {
  ...
  function apply(action) {
    ...
    // естественное остывание/рост
    state.load = clamp01(state.load - 0.08);
...
async function main() {
  ...
  if (!demo) {
    console.log('Usage: node evolution/puppeteer.js --demo [--steps N] [--json]');
    return;
  }
  ...
  if (json) {
    console.log(JSON.stringify({ stats: pup.stats(), log: log.map((l) => ({
  ...
  for (const l of log) {
    console.log(
      `  step ${String(l.action && log.indexOf(l) + 1).padStart(2)} ` +
...
module.exports = {
  ...
  _internals: { mean, stdDev, slope, round },
};
