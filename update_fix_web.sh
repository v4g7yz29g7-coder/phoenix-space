#!/bin/bash
set -e
echo "🔥 Исправляем веб-версию (Цвет + Отступ)..."

# 1. Перезаписываем SVG файлы с явным фиолетовым цветом (#a78bfa)
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100">
  <g fill="none" stroke="#a78bfa" stroke-width="1.5" stroke-linejoin="round">
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100">
  <g fill="none" stroke="#a78bfa" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
...
# 2. Переписываем JS на адаптивный расчёт высоты (vh)
cat > /home/ishidin/phoenix/public/js/sacred-carousel.js << 'ENDOFFILE'
/**
 * 🐦‍🔥 Глубинное парение — ФИНАЛЬНЫЙ ФИКС для веб-версии
 * Цвет и отступ подстроены под большие экраны, без ущерба для мобилок
 */
(function() {
    const sacredData = [
        { icon: '/shapes/bg3.svg' },
    ...
    function spawnSymbol() {
        ...
        // Адаптивный отступ: 15-20% от высоты экрана, но минимум 100px для мобилок
        const bottomOffset = Math.max(window.innerHeight * 0.15, 100) + Math.random() * 60;
        ...
        img.src = symbol.icon;
        img.alt = '';
        // Явный цвет SVG решает проблему черноты на веб-версии
        img.style.cssText = `
